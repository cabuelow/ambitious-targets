...
PR.m$PR <- rep(scenarios[j,1], nrow(PR.m))
PR.m$RR <- rep(scenarios[j,2], nrow(PR.m))
projections[[j]] <- PR.m
Pratepr.m <- do.call(rbind, ls2)
Pratepr.m$PR <- rep(scenarios[j,1], nrow(Pratepr.m))
Pratepr.m$RR <- rep(scenarios[j,2], nrow(Pratepr.m))
prot.rates[[j]] <- Pratepr.m
Rratepr.m <- do.call(rbind, ls3)
Rratepr.m$PR <- rep(scenarios[j,1], nrow(Rratepr.m))
Rratepr.m$RR <- rep(scenarios[j,2], nrow(Rratepr.m))
...
saveRDS(mang.proj, 'outputs/mangrove-projections-all.rds')
saveRDS(rest.rates.m, 'outputs/mangrove-restoration-rates-all.rds')
saveRDS(prot.rates.m, 'outputs/mangrove-protection-rates-all.rds')
