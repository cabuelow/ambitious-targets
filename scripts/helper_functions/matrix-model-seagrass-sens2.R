...
seagrass_protect3 <- function(x, PR.target, cons.start, target.year.protect, target.year, unprotect){
  ...
  x.sort$unprotect.year <- c(rep(1:years, each = protect.year + unprotect), rep(0, nrow(x.sort)-years*(protect.year+unprotect)))
  x <<- x.sort
}
  
seagrass_project3 <- function(site, M, LM, # initial states
                             ...
                             protect.year, unprotect.year){ # restoration target 
  ...
  int <- list(rep(initial.year, 2)) # initialise for first year
  for(k in 1:years){
    sf <- rep(initial.year+k, 2)
    int[[i+k]] <- sf
  }
  year <- do.call(c, int)
  ...
  int <- list(rep(cons.start, 2)) # initialise for first year
  for(k in 1:years){
    sf <- rep(cons.start+k, 2)
    int[[i+k]] <- sf
  }
  year <- do.call(c, int)
