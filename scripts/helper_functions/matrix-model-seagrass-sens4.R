...
seagrass_protect4 <- function(x, PR.target, cons.start, target.year.protect, sort){
  ...
  x <<- x.sort
    ...
    x.sort1 <- x %>% filter(loss<0) %>% arrange(desc(loss))
    x.sort2 <- x %>% filter(loss==0)
    x.sort <- rbind(x.sort1, x.sort2)
    row.select <- round(nrow(x.sort) * PR.target)
    x.sort$protect <- c(rep('Y', row.select), rep('N', nrow(x.sort)-row.select))
    protect.year <- round(row.select/years)
    x.sort$protect.year <- c(rep(1:years, each = protect.year), rep(0, nrow(x.sort)-years*protect.year))
...
seagrass_project5 <- function(site, M, LM, # initial states
