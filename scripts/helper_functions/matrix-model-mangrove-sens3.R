...
mangrove_project4 <- function(country, PF, UF, PD, UD, # initial states
                             ...
                             PR.target, RR.target){ # restoration target 
  ...
  # set restoration constant - ha in each year, proportion of restorable forest area in conservation start year
    ...
    if(PR > 0){
     ...
     # set transition probability matrix so that rate of deforestation increases in proportion to area protected
    ...
    trans[1,2] <- 0 
    trans[3,2] <- 0
    trans[4,2] <- 1 - exp((-abs(ewe)) + (-abs(commodities+(commodities*prop.inc))) + (-abs(npc+(npc*prop.inc))))
    trans[5,2] <- 1 - exp((-abs(erosion)) + (-abs(settle)))
    trans[1,3] <- 1 - exp(-abs(growth)) # PD to PF
    trans[2,3] <- 0
    trans[4,3] <- 0
