...
# to percent increases or decreases (10, 20, or 30%) in the initial condition of annual rate(s) of expansion
...
for(j in seq_along(rates)){
  ...
  datm <- mang %>% 
    mutate(Growth_rate_GMW = multm(Growth_rate_GMW))
  
  datp <- mang %>% 
    mutate(Growth_rate_GMW = multp(Growth_rate_GMW))
...
for(j in seq_along(rates)){
  ...
  datm <- seag %>% 
    mutate(growth = multm(growth))
  
  datp <- seag %>% 
    mutate(growth = multp(growth))
...
ggsave('outputs/sensitivity-global-trajectory-expansion-rates.png', width = 7, height = 5)
