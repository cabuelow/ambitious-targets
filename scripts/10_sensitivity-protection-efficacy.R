# test the sensitivity of extent change projections to variable protection effectiveness

library(tidyverse)
library(patchwork)
source('scripts/helper_functions/matrix-model-mangrove-sens.R')
source('scripts/helper_functions/matrix-model-seagrass-sens.R')
...
# vary protection efficacy

# mangroves, illegal deforestation in PAs can occur at a rate 10, 20, or 30 plus or minus the settlement rate of loss

rates <- c(0, 0.1, 0.2, 0.3)
biglist <- list()
...
for(j in seq_along(rates)){
  ...
  datm <- mang %>% 
    mutate(pa.deforest = multm(commodities_rate_10_16+npc_rate_10_16))
  
  datp <- mang %>% 
    mutate(pa.deforest = multp(commodities_rate_10_16+npc_rate_10_16))
  ...
  for(k in seq_along(dall)){
    ...
    for(i in 1:nrow(d)){
      ...
      mangrove_project2(country = sub$Country, PF = sub$Protected_forest_ha, UF = sub$Unprotected_forest_ha, 
                       ...
                       target.year.restore = 2050, project.year = 2050, pa.deforest = sub$pa.deforest,  # conservation scenario start year and target year
    ...
    for(i in 1:nrow(d)){
      ...
      mangrove_project2(country = sub$Country, PF = sub$Protected_forest_ha, UF = sub$Unprotected_forest_ha, 
                       ...
                       ewe = sub$ewe_rate_10_16, erosion = sub$erosion_rate_10_16, commodities = sub$commodities_rate_10_16, 
                       settle = sub$settle_rate_10_16, npc = sub$npc_rate_10_16, growth = sub$Growth_rate_GMW, # loss and growth rates
                       initial.year = 2016, cons.start = 2023, target.year.protect = 2030,  pa.deforest = sub$pa.deforest,
...
# seagrass, vary loss rates so that protection is not 100% effective
# 70, 80, 90% effective
...
for(j in seq_along(rates)){
  ...
  r <- rates[j]
    ...
    seagrass_protect(seag, PR.target = 0.3, target.year.protect = 2030, cons.start = 2023)
    
    for(i in 1:nrow(seag)){
      
      sub <- x[i,] # select a meadow
      
      seagrass_project2(site = sub$study_site, M = sub$area_start, LM = sub$restore_area, # initial states
                       ...
                       RR.target = 0, PR.target = 0.3, p.var = r, 
    ...
    for(i in 1:nrow(seag)){
      ...
      seagrass_project2(site = sub$study_site, M = sub$area_start, LM = sub$restore_area, # initial states
                       loss = sub$loss, growth = sub$growth, # loss and growth rates
                       cons.start = 2023, initial.year = 2000, target.year.protect = 2030, 
                       target.year.restore = 2050, project.year = 2050, p.var = r, # conservation start year and target year
...
df <- mutate(df, percent = ifelse(is.na(percent), 0, percent))
...
a <- ggplot() +
  ...
  geom_line(data = filter(dfsub.P, direction == 'c'), aes(x = year, y = area_ha)) +
...
b <- ggplot() +
  ...
  geom_line(data = filter(dfsub.R, direction == 'c'), aes(x = year, y = area_ha)) +
...
ggsave('outputs/sensitivity_protection-efficacy_global-trajectory.png', width = 7, height = 5)
