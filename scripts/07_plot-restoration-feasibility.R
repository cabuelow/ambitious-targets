# assess and plot the feasibility of required restoration each year
...
# and multiply by median size of restoration projects (Bayaktarov 2016)
...
# make inset barplots of restoration under most ambitious scenario

sea.R2 <- sea.R %>% filter(scenario == 'PR2') %>% 
  pivot_longer(cols = c(rest_ha_yr, rest_ha_yr_obs), 
               names_to = 'rate', values_to = 'val')

sea.R2$label <- recode(sea.R2$rate, rest_ha_yr = 'R', rest_ha_yr_obs = 'O')
...
mang.R2$label <- recode(mang.R2$rate, rest_ha_yr = 'R', rest_ha_yr_obs = 'O')
