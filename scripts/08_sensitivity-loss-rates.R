...
# to percent increases or decreases (10, 20, or 30%) in the initial condition of annual rate(s) of loss
...
# test sensitivity to increase or decrease in rates of loss by 10, 20, 30%
...
for(j in seq_along(rates)){
  ...
  datm <- mang %>% 
    mutate_at(c('erosion_rate_10_16', 'erosion_rate_10_16', 
                'ewe_rate_10_16', 'commodities_rate_10_16', 
                'npc_rate_10_16', 'settle_rate_10_16'), multm)
  
  datp <- mang %>% 
    mutate_at(c('erosion_rate_10_16', 'erosion_rate_10_16', 
                'ewe_rate_10_16', 'commodities_rate_10_16', 
                'npc_rate_10_16', 'settle_rate_10_16'), multp)
  ...
  for(k in seq_along(dall)){
    ...
    for(i in 1:nrow(d)){
      ...
      mangrove_project(country = sub$Country, PF = sub$Protected_forest_ha, UF = sub$Unprotected_forest_ha, 
                   ...
                   target.year.restore = 2050, project.year = 2050,
                   PR.target = 0.3, RR.target = 0)
...
for(j in seq_along(rates)){
  ...
  datm <- seag %>% 
    mutate(loss = multm(loss))
  
  datp <- seag %>% 
    mutate(loss = multp(loss))
  ...
  for(k in seq_along(dall)){
    ...
    for(i in 1:nrow(seag)){
      ...
      seagrass_project(site = sub$study_site, M = sub$area_start, LM = sub$restore_area,
                       ...
                       cons.start = 2023, initial.year = 2000, target.year.protect = 2030, 
                       target.year.restore = 2050, project.year = 2050,
                       RR.target = 0.5, PR.target = 0, protect.year = sub$protect.year)
      ls[[i]] <- df.full
      ls2[[i]] <- rest.rate
    }
...
ggsave('outputs/sensitivity-global-trajectory-loss-rates.png', width = 7, height = 5)
