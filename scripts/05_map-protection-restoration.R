...
# calculate summary stats of required protection and restoration each year under ambitious conservation scenario

mang.prot <- prot.rates.m %>% 
  filter(scenario == 'PR2') %>% 
  summarise(min_ha = min(prot_ha_yr),
            mean_ha = mean(prot_ha_yr),
            median_ha = median(prot_ha_yr),
            max_ha = max(prot_ha_yr)) %>% 
  mutate(Ecosystem = 'Mangrove',
         Scenario = 'PR2',
         Action = 'Protection', 
         Country_max_ha = filter(prot.rates.m, scenario == 'PR2' & prot_ha_yr == max(prot_ha_yr))[1,1])

mang.rest <- rest.rates.m %>% 
  filter(scenario == 'PR2') %>% 
  summarise(min_ha = min(rest_ha_yr),
            mean_ha = mean(rest_ha_yr),
            median_ha = median(rest_ha_yr),
            max_ha = max(rest_ha_yr)) %>% 
  mutate(Ecosystem = 'Mangrove',
         Scenario = 'PR2',
         Action = 'Restoration', 
         Country_max_ha = filter(rest.rates.m, scenario == 'PR2' & rest_ha_yr == max(rest_ha_yr))[1,1])

seag.prot <- prot.rates.s %>% 
  filter(scenario == 'PR2') %>% 
  rename(study_site = site) %>% 
  full_join(select(data.frame(seag.sf), study_site, SOVEREIGN1)) %>% 
  mutate(prot_ha_yr = ifelse(is.na(prot_ha_yr), 0, prot_ha_yr)) %>% 
  group_by(SOVEREIGN1) %>%
  summarise(prot_ha_yr = sum(prot_ha_yr)) %>%
  summarise(min_ha = min(prot_ha_yr),
            mean_ha = mean(prot_ha_yr),
            median_ha = median(prot_ha_yr),
            max_ha = max(prot_ha_yr)) %>% 
  mutate(Ecosystem = 'Seagrass',
         ...
         Country_max_ha = prot.rates.s %>% 
           ...
           group_by(SOVEREIGN1) %>%
           summarise(prot_ha_yr = sum(prot_ha_yr)) %>% 
           filter(prot_ha_yr == max(prot_ha_yr)) %>% 
           select(SOVEREIGN1) %>% 
           mutate(SOVEREIGN1 = as.character(SOVEREIGN1)) %>% 
           as.character())

seag.rest <- rest.rates.s %>% 
  filter(scenario == 'PR2') %>% 
  rename(study_site = site) %>% 
  full_join(select(data.frame(seag.sf), study_site, SOVEREIGN1)) %>% 
  mutate(rest_ha_yr = ifelse(is.na(rest_ha_yr), 0, rest_ha_yr)) %>% 
  group_by(SOVEREIGN1) %>%
  ...
  mutate(Ecosystem = 'Seagrass',
         ...
         Country_max_ha = rest.rates.s %>% 
           ...
           group_by(SOVEREIGN1) %>%
           summarise(rest_ha_yr = sum(rest_ha_yr)) %>% 
           filter(rest_ha_yr == max(rest_ha_yr)) %>% 
           select(SOVEREIGN1) %>% 
           mutate(SOVEREIGN1 = as.character(SOVEREIGN1)) %>% 
           as.character())

sum.stats <- rbind(mang.prot, mang.rest, seag.prot, seag.rest)

write.csv(sum.stats, 'outputs/summary-statistics-cons-required.csv', row.names = F)
...
write.csv(mang.P, 'outputs/mangrove-protection-proportion.csv', row.names = F)
...
write.csv(sea.R, 'outputs/seagrass-restoration-proportion.csv', row.names = F)
